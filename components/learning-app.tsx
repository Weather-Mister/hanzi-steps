...
import {Character,characters,characterOrder,findLesson,GrammarRule,grammarRules,Lesson,lessonAvailable,lessons,phrases,Session,shuffled,Step,Unit,units,books,vocabulary,wordMeaning,unitLibraryCharacters,completedLessonIds,previousLessonLengths} from '@/lib/curriculum';
...
function Phonetics({character,prefs}:{character:Character;prefs:Preferences}){return <>{prefs.pinyin&&<span className="pinyin">{character.pinyin}</span>}{prefs.zhuyin&&<span className="zhuyin" lang="zh-Hant-TW">{character.zhuyin}</span>}</>}
...
function LearningExperience({userKey,accountPanel,signInPanel}:AppProps){
 ...
 <SmartPractice key={userKey} open={practiceOpen} onOpenChange={setPracticeOpen} completed={completed} theme={currentTheme} mastery={practiceMastery} megaMastery={megaMastery} startMode={practiceStart} onOpenMegaChallenge={()=>setMegaOpen(true)}/>
