...
import {progressRequest} from './progress-transport';
...
async function request(method:'GET'|'PUT',account:string,body?:Session){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),12000);try{const r=await progressRequest(method,account,body,controller.signal);if(r.status===401)throw new ProgressRequestError('Sign in to sync your progress.',401);const data=await r.json() as {error?:string;sessions?:unknown;saved?:boolean;studyDays?:unknown};if(!r.ok)throw new ProgressRequestError(data.error||'Progress is temporarily unavailable.',r.status);if(method==='PUT'&&data.saved!==true)throw new Error('The checkpoint was not confirmed.');return data}finally{clearTimeout(timer)}}
export function useProgress(userKey:string){
 ...
 const load=useCallback(async()=>{
  ...
  try{const {sessions:rows,studyDays}=await request('GET',userKey);if(!Array.isArray(rows))throw new Error('Progress could not load.');const remote:Session[]=rows.filter(validSession);let combined=remote;
